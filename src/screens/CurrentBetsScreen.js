...
async function moveBet(currentdoc, game){
  ...
  if (homeWin(game) && currentdoc.teamBetOn == game['teams']['home']['name']) {
    ...
    let balanceUpdate = 0;
    let betWins = 0;
    await firestore().collection('users').doc(global.currentuid).get()
      .then((users) => {
        var data = users.data();
        balanceUpdate = data.balance + Number(currentdoc.moneyToBePaid);
        betWins = data.betWins + 1;
        firestore().collection('users').doc(global.currentuid).set({balance: balanceUpdate, betWins: betWins}, {merge: true});
      }).catch((error) => {
  ...
  await firestore().collection('users').doc(global.currentuid).collection('pastbets').doc(currentdoc.dateTimeID).set(data, {merge:true}); // set data in past bets
  await firestore().collection('users').doc(global.currentuid).collection('activebets').doc(currentdoc.dateTimeID).delete() // delete data from active bets
    .then(() => {
...
async function checkBets(games){
  ...
  const markers = [];
  await firestore().collection('users').doc(global.currentuid).collection('activebets').get()
    .then((querySnapshot) => {
  ...
  for (let i = 0; i < markers.length; i++){
    for (let j = 0; j < games.length; j++){
      console.log('hi');
      if((games[j]['status']['short'] == "FT" && games[j]['id'] == markers[i].gameID) || (
      games[j]['status']['short'] == "AOT" && games[j]['id'] == markers[i].gameID)) {
